import {
  ...
  export async function classifyResponsibilityLevel(demographicInfo) {
    ...
    score += (normalizedDecisionLevel === "strategic" ? 1 : normalizedDecisionLevel === "tactical" ? 0.5 : 0) * weights.decisionLevel;
    ...
    const levelIndex = Math.min(Math.floor(score * levels.length), levels.length - 1);
    const classifiedLevel = levels[levelIndex];
  
    // Find the matching level in responsibilityLevelsData
    const matchedLevel = responsibilityLevelsData.find(level => 
